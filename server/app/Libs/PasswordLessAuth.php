...
class PasswordLessAuth
{
    ...
    private $email_subject = ' Access link';
    ...
    {
        try {
            $db_host = env('DB_HOST', 'localhost'); //Database hostname (usually 'localhost')
            $db_username = env('DB_USERNAME', 'admin'); //Database username
            $db_password = env('DB_PASSWORD', 'admin'); //Database password
            $db_name = env('DB_DATABASE', 'bitcoin'); //Database name

            $this->pdo = new PDO("mysql:host=$db_host;dbname=$db_name", $db_username, $db_password);
            $this->pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $this->pdo->setAttribute(PDO::ATTR_DEFAULT_FETCH_MODE, PDO::FETCH_OBJ);

            $this->login_base_url = env('CLIENT_HOST', 'localhost') . '/#/auth/';
            $this->email_subject = env('CLIENT_HOST', '') . $this->email_subject;
    ...
    public function request_login_code($email)
    {
        ...
        if (!$validation['is_valid']) {
            ...
            if ($record_created) {
                ...
                $body = str_replace('[loginURL]', $login_url, $this->email_html_body);
                MailSpool::addMail($email, $this->email_subject, $body);
            } else {
                $response['was_successfull'] = false;
